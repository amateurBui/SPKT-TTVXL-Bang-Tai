...
void USART2_IRQHandler(void)
{
	if(USART2->DR == ';')
	{
        sprintf(buffer, "Received command: %d \n", value);
        send_string(buffer);
        count_rx = 0;// RESET moi lan nhap len Terminal
...
void TIM2_IRQHandler(void)
{
 ...
 else if ((covat == 0) && (mode == 1) && (dem<=4000)){
	 stateMotor = 0;
 }
 else if ((covat == 0) && (mode == 1) && (dem>4000)&& (state_m1 == 0)){
	 stateMotor = 1;
 }
 else if ((state_m1 == 1) && (mode == 1)){
	 stateMotor = 0;
 }
}
...
//		send_string("Ko Vat\n");
	...
	else if ((buttonStart == 0) && (mode == 1) && (state4 == 0)){// auto
		stateMotor = 0;
		state4 =1 ;
