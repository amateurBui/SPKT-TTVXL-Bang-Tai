...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
	Clock_Init();
	GPIOA->MODER |= (1<<5); //PA9-TX
	GPIOA->MODER |= (1<<7);	 //PA9-RX
...
//	GPIOE->OTYPER = 0 ; //Push-pull mode
	...
	GPIOA->AFR [0] = (7<<8) | (7<<12);
	GPIOA->OTYPER &=~(1<<10);
	GPIOB->MODER |= (1<<6) | (1<<10);
	USART2->BRR = (104 << 4) | (3<<0); // baudrate 9
	USART2->CR1 |= (1<<2) |(1<<3)|(1<<5) |(1<<13); //
	GPIOE->MODER |= (1<<0) | (1<<2) | (1<<3);
	GPIOE->ODR &=~(1<<3);
	GPIOE->ODR &=~(1<<2);
	GPIOB->ODR &=~(1<<0);
	TIM2->PSC = 1999;// 1s
...
void Conveyor()
{
	...
	else if ((GPIOA->IDR & (1<<15)) == 0)
	{
		stateMotor = 1;
	}
	if (stateMotor == 0)      //AUTO
	{
		GPIOA->ODR &= ~(1<<7); // turn on LED 1
		GPIOB->ODR |= (1<<2); //turn off LED 2
		if ((GPIOD->IDR & (1<<13)) != 0)
		{
			GPIOB->ODR &= ~(1<<0);
